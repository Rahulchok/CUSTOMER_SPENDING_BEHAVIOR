...
##PROBLEM STATEMENT :Customer Behavior of purchases for Walmat
"""
...
##Changeing(reducing) the population sample size
"""
...
fig=plt.figure(figsize=(10,5))
plt.subplot(1,2,1)
sns.histplot(data=mat_df[mat_df['Marital_Status']=='Unmarried'],x="Purchase",bins=35)
plt.title('Number of purchase for Unmarried customer')
plt.subplot(1,2,2)
sns.histplot(data=mat_df[mat_df['Marital_Status']=='Maried'],x="Purchase",bins=35)
...
#city_cat=df.groupby(["City_Category","Product_Category"])["Purchase"].sum().round(2).reset_index().rename(columns={"Purchase":"Total_Purchase"})
city_cat_pivot=pd.pivot_table(df,index=["Product_Category"],columns="City_Category",values="Purchase",aggfunc=np.sum).reset_index()
city_cat_pivot.columns=["Product_Category","City_Category_A","City_Category_B","City_Category_C"]
city_cat_pivot

cat=city_cat.sort_values(by=["City_Category","Total_Purchase"],ascending=[True,False])
cat.index=[i for i in range(cat.shape[0])]
cat

"""for A B or C catagory state 1,5,8 product catagory are top saleeing product"""

city_cat_pivot=pd.pivot_table(df,index=["Gender"],columns="City_Category",values="Purchase",aggfunc=np.mean).reset_index()
city_cat_pivot.columns=["Product_Category","City_Category_A","City_Category_B","City_Category_C"]
city_cat_pivot

"""Male customers living in City_Category C spend more money than other male customers living in B or A"""
...
mat_cat=data.groupby(["Gender","Marital_Status"])["User_ID"].nunique().reset_index().rename(columns={"Purchase":"user"})
mat_cat

"""both of male and female unmaried customers are more
